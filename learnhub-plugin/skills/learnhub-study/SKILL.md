...
主持完整的学-练-评闭环。数据权威在 SQLite，评分只走 learnhub_* 工具。
...
3. **陪练**：用 lesson 包里的 exercises，逐题让用户作答；`learnhub_check` 判卷——sympy 题（含数值容差）与 choice 选择题（选项字母）直接给对错；human 题返回参考答案让用户自评；ai 开放题返回 `{judge:"ai", q, answer:评分要点}` 且不记录——由你按评分要点判卷（对/半对/错 + 反馈），再用 `learnhub` 工具执行 `learn record-attempt <node> ex<N> --answer <作答> --judge ai --correct 0|1 --feedback <评语> --course <课程>` 落库。
4. **记录评分**：`learnhub_grade`（node 支持 `课程/节点` 消歧，直写权威库+投影）即时入库，无需结算。
...
- agent 侧用 `learnhub_generate`（course+node），产出同样是 draft，需引导用户在面板人审（或 `content review`）后才能进入学习。超纲靠提示词与上下文包的「领域边界/禁止使用的概念」约束，质检门只警告不拦截，人审是最后关口。
