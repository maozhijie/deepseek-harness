...
1. **分析**：调用 `learnhub_graph_analyze`（`course` 指定课程）——返回结构统计、遗忘热点、不可达节点、瓶颈节点，这是优化的依据。
...
3. **提交门禁**：`learnhub_graph_propose`，`kind: "edit"`。结构检查会把 ops 代入模拟图验证：引用断边、改名冲突、环都会被拒；按报错修正重交。
