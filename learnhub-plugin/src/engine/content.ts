...
`,
  ...
  async sectionApply(
    ...
    const newBody = Content.assembleBody(body, sections, new Map([[entry.title, split.body.trim()]]))
    ...
    return { version, title: entry.title, hints: Content.encBackfeedHints(graph, node, split.body) }
  }
