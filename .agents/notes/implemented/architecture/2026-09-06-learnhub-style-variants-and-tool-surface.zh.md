...
e2e 风格步骤断言五个新内置、退役的 `课程生成` 不再内置、苏格拉底变体仍带节级标记（`## 类型：标题`）、落盘与未知风格 fail loud；smoke（真实 vault）核对同一清单。图谱步骤新增 set_enc 往返落盘、analyze schema 字段断言、enc 断边拒绝与提案列表——后者拚住了 schema 初版把 `encOf` 元组直接透传、与声明的 `{node,w}` 对象不符的问题。探索工具有独立断言：节点详情字段/succ/enc、区块浏览计数、路径链与反向无关、单题含答案与未知 qid fail loud（首版断言用了被 rename 联动改名的节点而报空库——题库随迁语义的活样本）。加载冒烟有反证：临时移除 `additionalProperties` 后冒烟 FAIL 报出与 dsh 启动崩溃完全相同的错误，恢复后全绿。`npm run build`、`npm run check`、UI `tsc --noEmit`、e2e 全绿。
